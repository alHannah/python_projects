...
def processing_ingredients(customer_order):
    if make_order in MENU:
        if make_order == "espresso":
            resources["water"] = resources["water"] - MENU[make_order]["ingredients"]["water"]
            resources["coffee"] = resources["coffee"] - MENU[make_order]["ingredients"]["coffee"]
        else:
            resources["water"] = resources["water"] - MENU[make_order]["ingredients"]["water"]
            resources["milk"] = resources["milk"] - MENU[make_order]["ingredients"]["milk"]
            resources["coffee"] = resources["coffee"] - MENU[make_order]["ingredients"]["coffee"]
...
while operating:
    ...
    # TODO 4. Ask for another order Turn off by saying "off" "report" to show resource
        ...
        # TODO 1.2 Check for enough ingredients "Sorry there is not enough {}"
        depleted_ingredient = []
        for specific_ingredient in resources:
            ingredient_left = resources[specific_ingredient]
        if ingredient_left <= 0:
            depleted_ingredient.append(specific_ingredient)
            print(f"Sorry there is not enough {depleted_ingredient}.")
            operating = False
        else:
